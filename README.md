...
Workflow `.github/workflows/deploy.yml` автоматически собирает и тестирует образ после push в `main`. Развертывание ожидает ручного подтверждения в GitHub Environment `production`. После подтверждения workflow устанавливает актуальный `deploy/deploy.sh`, проверяет свободное место с учетом полного несжатого размера нового образа, расширения при импорте и размера базы данных, а затем передает сжатый архив потоком по SSH. VPS распаковывает поток прямо в `docker load`, не создавая архив на диске. Активный образ сохраняется для отката. Если запуск приложения или проверка `/health` завершается ошибкой, скрипт возвращает предыдущий образ и отдельно проверяет его `/health`. Дамп базы данных создается перед развертыванием, но автоматически при откате приложения не восстанавливается.
