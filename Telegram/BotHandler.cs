    ...
    BalanceService balanceService)
{
    ...
    private async Task SaveManagedParticipant(long userId, string? paymentDetails, CancellationToken ct)
    {
        ...
        var effectiveNames = (await Participants(data.GroupId, ct)).Select(x => x.DisplayName);
        if (effectiveNames.Any(x => string.Equals(x, data.ManagedName, StringComparison.OrdinalIgnoreCase)))
    ...
    private async Task SaveExpense(long userId, FlowData data, CancellationToken ct)
    {
        ...
        var activeMemberIds = await db.GroupParticipants
