...
export function ExpenseDetailPage() {
  ...
  const remove = useMutation({
    mutationFn: () => endpoints.deleteExpense(groupId, expenseId),
    onSuccess: () => {
  ...
  if (query.isError) return <Page title="Покупка"><ErrorState error={query.error} retry={() => query.refetch()} /></Page>
  ...
  return <Page title={expense.description} eyebrow={new Intl.DateTimeFormat('ru-RU', { dateStyle: 'long' }).format(new Date(expense.createdAt))}>
    ...
    <Dialog open={confirm} onClose={() => { if (!remove.isPending) setConfirm(false) }}><DialogTitle>Удалить покупку?</DialogTitle><DialogContent><Typography color="text.secondary">Баланс группы будет пересчитан.</Typography><FormError message={remove.error?.message} /></DialogContent><DialogActions><Button onClick={() => setConfirm(false)} disabled={remove.isPending}>Отмена</Button><Button color="error" onClick={() => { if (!remove.isPending) remove.mutate() }} disabled={remove.isPending}>{remove.isPending ? 'Удаляем…' : 'Удалить'}</Button></DialogActions></Dialog>
  </Page>
}
